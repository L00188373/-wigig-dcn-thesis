...
struct SimConfig {
 ...
 // Blockage simulation parameters
    bool enableBlockage = false;
    double blockageStartTime = 10.0;
    double blockageDuration = 5.0;
...
int main(int argc, char *argv[])
{
    ...
    cmd.AddValue("blockage", "Enable link blockage", config.enableBlockage);
    cmd.AddValue("redundancy", "Enable redundant links", config.enableRedundancy);    
    cmd.Parse(argc, argv);
...
// Add redundant link if enabled (ToR 1 ↔ ToR 3)
    if (config.enableWireless && config.enableRedundancy) {
        NS_LOG_INFO("Creating REDUNDANT wireless link:");
        NetDeviceContainer wireless13 = p2pWireless.Install(torSwitches.Get(1), 
                                                           torSwitches.Get(3));
        address.SetBase("172.16.3.0", "255.255.255.252");
        address.Assign(wireless13);
        NS_LOG_INFO("  ToR 1 ↔ ToR 3: " << config.wirelessRate << " (redundant path)\n");
    }
    
    // Schedule blockage events
    if (config.enableBlockage && config.enableWireless) {
        Simulator::Schedule(Seconds(config.blockageStartTime), [&]() {
            NS_LOG_INFO("\n╔═══════════════════════════════════════╗");
            NS_LOG_INFO("║  BLOCKAGE EVENT at " << Simulator::Now().GetSeconds() << "s");
            NS_LOG_INFO("║  ToR 2 ↔ ToR 3 link BLOCKED");
            NS_LOG_INFO("╚═══════════════════════════════════════╝\n");
        });
        
        Simulator::Schedule(Seconds(config.blockageStartTime + config.blockageDuration), [&]() {
            NS_LOG_INFO("\n╔═══════════════════════════════════════╗");
            NS_LOG_INFO("║  BLOCKAGE CLEARED at " << Simulator::Now().GetSeconds() << "s");
            NS_LOG_INFO("║  ToR 2 ↔ ToR 3 link RESTORED");
            NS_LOG_INFO("╚═══════════════════════════════════════╝\n");
        });
    }
